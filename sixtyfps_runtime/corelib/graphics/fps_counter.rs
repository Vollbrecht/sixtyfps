...
impl FPSCounter {
    ...
    pub fn measure_frame_rendered(
        ...
        if self.output_overlay {
            let label = crate::items::Text {
                width: crate::properties::Property::new(100.),
                height: crate::properties::Property::new(100.),
                text: crate::properties::Property::new(
                    format!("FPS: {}", self.frame_times.borrow().len()).into(),
                ),
                color: crate::properties::Property::new(
                    crate::Color::from_rgb_u8(0, 128, 128).into(),
                ),
                ..Default::default()
            };
            let label = unsafe { core::pin::Pin::new_unchecked(&label) };
            renderer_for_overlay.draw_text(label);
        }
    }
}
